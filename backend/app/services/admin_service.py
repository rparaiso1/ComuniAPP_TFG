...
class AdminService:
    ...
    def dashboard(self, org_ids: List[UUID]) -> dict:
        ...
        user_stats = (
            self.db.query(
                ...
                func.count(
                    case(
                        (User.is_active.is_(True), UserOrganization.user_id),
                    ).distinct()
                ).label("active"),
            )
        ...
        total_zones = (
            ...
            .filter(Zone.organization_id.in_(org_ids), Zone.is_active.is_(True))
            .scalar() or 0
        )
        pending_invitations = (
    ...
    def list_users(self, org_ids: List[UUID], role: Optional[str] = None, active_only: bool = False, skip: int = 0, limit: int = 50) -> List[dict]:
        ...
        if active_only:
            query = query.filter(User.is_active.is_(True), UserOrganization.is_active.is_(True))
        if role:
