...
- `start-all.ps1`: DB + backend + Flutter Web (Chrome)
- `start-android.ps1`: DB + backend + Flutter en emulador Android
- `start-db.ps1`: solo PostgreSQL + pgAdmin
- `start-backend.ps1`: solo backend FastAPI
- `start-flutter.ps1`: solo frontend Flutter Web

### Flutter en Android (emulador)

```powershell
./start-android.ps1
```

Lanza automáticamente el emulador `Emulador_QA`, espera el boot completo, libera memoria en background y ejecuta la app. Opciones:

```powershell
# Dispositivo adb concreto
./start-android.ps1 -Device emulator-5554

# Dispositivo físico (usa la IP local de la máquina)
./start-android.ps1 -Physical
```

> Requiere: Android Studio con un AVD llamado `Emulador_QA` y `adb` en el PATH.
